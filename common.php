...
class KiwiCfContactForm
{

    const post_type = 'kiwi_cf_contact_form';
    ...
    public static function register_post_type()
    {
        register_post_type(self::post_type, array(
            'labels' => array(
                'name' => __('Contact Forms', 'kiwi-contact-form'),
                'singular_name' => __('Contact Form', 'kiwi-contact-form'),
            ),
            'rewrite' => false,
            'query_var' => false,
            'public' => false,
            ...
            'capabilities' => array(
                'edit_post' => 'kiwi_cf_edit_contact_form',
                'read_post' => 'kiwi_cf_read_contact_form',
                'delete_post' => 'kiwi_cf_delete_contact_form',
                'edit_posts' => 'kiwi_cf_edit_contact_forms',
                'edit_others_posts' => 'kiwi_cf_edit_contact_forms',
                'publish_posts' => 'kiwi_cf_edit_contact_forms',
                'read_private_posts' => 'kiwi_cf_edit_contact_forms',
            ),
        ));
    }
    ...
    public static function get_template($args = '')
    {
        ...
        if ($locale) {
            $mo_orig = $l10n['kiwi-contact-form'];
            kiwi_cf_load_textdomain($locale);
        }

        self::$current = $contact_form = new self;
        $contact_form->title =
            ($title ? $title : __('Untitled', 'kiwi-contact-form'));
        $contact_form->locale = ($locale ? $locale : get_user_locale());

        $properties = $contact_form->get_properties();

        foreach ($properties as $key => $value) {
            $properties[$key] = KiwiCfContactFormTemplate::get_default($key);
        }

        $contact_form->properties = $properties;

        $contact_form = apply_filters('kiwi_cf_contact_form_default_pack',
            $contact_form, $args);

        if (isset($mo_orig)) {
            $l10n['kiwi-contact-form'] = $mo_orig;
        }
    ...
    private static function generate_unit_tag($id = 0)
    {
        ...
        if (in_the_loop()) {
            $unit_tag = sprintf('kiwi-f%1$d-p%2$d-o%3$d',
                absint($id),
                get_the_ID(),
                $global_count
            );
        } else {
            $unit_tag = sprintf('kiwi-f%1$d-o%2$d',
                absint($id),
                $global_count
            );
        }
    ...
    private function __construct($post = null)
    {
        ...
        do_action('kiwi_cf_contact_form', $this);
    }

    public function __get($name)
    {
        $message = __('<code>%1$s</code> property of a <code>KiwiContactForm</code> object is <strong>no longer accessible</strong>. Use <code>%2$s</code> method instead.', 'kiwi-contact-form');
    ...
    public function get_properties()
    {
        ...
        $properties = (array)apply_filters('kiwi_cf_contact_form_properties',
            $properties, $this);
    ...
    public function set_title($title)
    {
        ...
        if ('' === $title) {
            $title = __('Untitled', 'kiwi-contact-form');
        }
    ...
    {
        if (kiwi_cf_is_valid_locale($this->locale)) {
            return $this->locale;
        } else {
            return '';
        }
    ...
    public function set_locale($locale)
    {
        ...
        if (kiwi_cf_is_valid_locale($locale)) {
            $this->locale = $locale;
        } else {
            $this->locale = 'en_US';
        }
    ...
    {
        if (!Kiwi_CF_Submission::get_instance()) {
            return false;
        }

        if (empty($_POST['_kiwi_cf_unit_tag'])) {
            return false;
        }

        return $this->unit_tag() === $_POST['_kiwi_cf_unit_tag'];
    }
    ...
    public function form_html($args = '')
    {
        ...
        if ('raw_form' == $args['output']) {
            return '<pre class="kiwi-raw-form"><code>'
                . esc_html($this->prop('form')) . '</code></pre>';
        }

        if ($this->is_true('subscribers_only')
            and !current_user_can('kiwi_cf_submit', $this->id())) {
            $notice = __(
                "This contact form is available only for logged in users.",
                'kiwi-contact-form');
            $notice = sprintf(
                '<p class="kiwi-subscribers-only">%s</p>',
                esc_html($notice));

            return apply_filters('kiwi_cf_subscribers_only_notice', $notice, $this);
        }
        ...
        $url = apply_filters('kiwi_cf_form_action_url', $url);

        $id_attr = apply_filters('kiwi_cf_form_id_attr',
            preg_replace('/[^A-Za-z0-9:._-]/', '', $args['html_id']));

        $name_attr = apply_filters('kiwi_cf_form_name_attr',
        ...
        $class = implode(' ', $class);
        $class = apply_filters('kiwi_cf_form_class_attr', $class);

        $enctype = apply_filters('kiwi_cf_form_enctype', '');
        $autocomplete = apply_filters('kiwi_cf_form_autocomplete', '');

        $novalidate = apply_filters('kiwi_cf_form_novalidate',
            kiwi_cf_support_html5());

        $atts = array(
            'action' => esc_url($url),
            'method' => 'post',
            'class' => $class,
            'enctype' => kiwi_cf_enctype_value($enctype),
    ...
    private function form_hidden_fields()
    {
        $hidden_fields = array(
            '_kiwi' => $this->id(),
            '_kiwi_cf_version' => KIWI_CF_VERSION,
            '_kiwi_cf_locale' => $this->locale(),
            '_kiwi_cf_unit_tag' => $this->unit_tag(),
            '_kiwi_cf_container_post' => 0,
        );

        if (in_the_loop()) {
            $hidden_fields['_kiwi_cf_container_post'] = (int)get_the_ID();
        }

        if ($this->nonce_is_active() && is_user_logged_in()) {
            $hidden_fields['_wpnonce'] = kiwi_cf_create_nonce();
        }

        $hidden_fields += (array)apply_filters(
            'kiwi_cf_form_hidden_fields', array());
    ...
    public function form_response_output()
    {
        ...
        if ($this->is_posted()) { // Post response output for non-AJAX
            ...
            switch ($status) {
                case 'validation_failed':
                    $class .= ' kiwi-validation-errors';
                    break;
                case 'acceptance_missing':
                    $class .= ' kiwi-acceptance-missing';
                    break;
                case 'spam':
                    $class .= ' kiwi-spam-blocked';
                    break;
                case 'aborted':
                    $class .= ' kiwi-aborted';
                    break;
                case 'mail_sent':
                    $class .= ' kiwi-mail-sent-ok';
                    break;
                case 'mail_failed':
                    $class .= ' kiwi-mail-sent-ng';
                    break;
                default:
                    $class .= sprintf(' kiwi-custom-%s',
                        preg_replace('/[^0-9a-z]+/i', '-', $status)
                    );
            }
        } else {
            $class .= ' kiwi-display-none';
        }
        ...
        $output = apply_filters('kiwi_cf_form_response_output',
    ...
    public function validation_error($name)
    {
        ...
        $error = sprintf(
            '<span role="alert" class="kiwi-not-valid-tip">%s</span>',
            esc_html($error));

        return apply_filters('kiwi_cf_validation_error', $error, $name, $this);
    }
    ...
    {
        $manager = KiwiCfFormTagsManager::get_instance();
        $form = $this->prop('form');

        if (kiwi_cf_autop_or_not()) {
            $form = $manager->normalize($form);
            $form = kiwi_cf_autop($form);
        }
    ...
    {
        kiwi_cf_deprecated_function(__METHOD__, '4.6',
            'KiwiContactForm::replace_all_form_tags');
    ...
    {
        kiwi_cf_deprecated_function(__METHOD__, '4.6',
            'KiwiCfContactForm::scan_form_tags');
    ...
    {
        return apply_filters('kiwi_cf_form_elements',
    ...
    public function collect_mail_tags($args = '')
    {
        ...
        return apply_filters('kiwi_cf_collect_mail_tags', $mailtags, $args, $this);
    }
    ...
    public function submit($args = '')
    {
        ...
        $submission = Kiwi_CF_Submission::get_instance($this, array(
        ...
        do_action('kiwi_cf_submit', $this, $result);
    ...
    public function filter_message($message, $status = '')
    {
        ...
        $message = kiwi_cf_mail_replace_tags($message, array('html' => true));
        $message = apply_filters('kiwi_cf_display_message', $message, $status);
    ...
    {
        $is_active = KIWI_CF_VERIFY_NONCE;

        if ($this->is_true('subscribers_only')) {
            $is_active = true;
        }

        return (bool)apply_filters('kiwi_cf_verify_nonce', $is_active, $this);
    }
        ...
        if ($post_id) {
            foreach ($props as $prop => $value) {
                update_post_meta($post_id, '_' . $prop,
                    kiwi_cf_normalize_newline_deep($value));
            }
            ...
            if ($this->initial()) {
                $this->id = $post_id;
                do_action('kiwi_cf_after_create', $this);
            } else {
                do_action('kiwi_cf_after_update', $this);
            }

            do_action('kiwi_cf_after_save', $this);
        }
    ...
    public function copy()
    {
        ...
        return apply_filters('kiwi_cf_copy', $new, $this);
    }
    ...
    public function shortcode($args = '')
    {
        ...
        if ($args['use_old_format']) {
            ...
            $shortcode = sprintf('[kiwi-contact-form id="%1$d" title="%2$s"]',
                $this->id, $title);
        }

        return apply_filters('kiwi_cf_contact_form_shortcode',
            $shortcode, $args, $this);
    }
}
