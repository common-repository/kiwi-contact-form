...
class KiwiCfConfigValidator {
	...
	public function get_default_message( $code ) {
		switch ( $code ) {
			case self::error_maybe_empty:
				return __( "There is a possible empty field.", 'kiwi-contact-form' );
			case self::error_invalid_mailbox_syntax:
				return __( "Invalid mailbox syntax is used.", 'kiwi-contact-form' );
			case self::error_email_not_in_site_domain:
				return __( "Sender email address does not belong to the site domain.", 'kiwi-contact-form' );
			case self::error_html_in_message:
				return __( "HTML tags are used in a message.", 'kiwi-contact-form' );
			case self::error_multiple_controls_in_label:
				return __( "Multiple form controls are in a single label element.", 'kiwi-contact-form' );
			case self::error_invalid_mail_header:
				return __( "There are invalid mail header fields.", 'kiwi-contact-form' );
			case self::error_deprecated_settings:
				return __( "Deprecated settings are used.", 'kiwi-contact-form' );
			default:
				return '';
		}
	}
	...
	public function validate() {
		...
		do_action( 'kiwi_cf_config_validator_validate', $this );
	...
	public function replace_mail_tags_with_minimum_input( $matches ) {
		...
		if ( $form_tags ) {
			$form_tag = new KiwiCfFormTag( $form_tags[0] );
			...
			if ( kiwi_cf_form_tag_supports( $form_tag->type, 'selectable-values' ) ) {
				if ( $form_tag->pipes instanceof Kiwi_CF_Pipes ) {
				...
				if ( $last_item and kiwi_cf_is_mailbox_list( $last_item ) ) {
			...
			// for back-compat
			$field_name = preg_replace( '/^kiwi\./', '_', $field_name );
	...
	public function detect_multiple_controls_in_label( $section, $content ) {
		...
		if ( preg_match_all( $pattern, $content, $matches ) ) {
			$form_tags_manager = KiwiCfFormTagsManager::get_instance();
			...
			foreach ( $matches[1] as $insidelabel ) {
				...
				foreach ( $tags as $tag ) {
					$is_multiple_controls_container = kiwi_cf_form_tag_supports(
						$tag->type, 'multiple-controls-container' );
					$is_zero_controls_container = kiwi_cf_form_tag_supports(
			...
			return $this->add_error( $section,
				...
				array(
					'message' =>
						...
						__( "Unavailable names (%names%) are used for form controls.", 'kiwi-contact-form' ),
	...
	public function detect_unavailable_html_elements( $section, $content ) {
		...
		if ( preg_match( $pattern, $content ) ) {
			return $this->add_error( $section,
				...
				array(
					'message' => __( "Unavailable HTML elements are used in the form template.", 'kiwi-contact-form' ),
	...
	public function validate_mail( $template = 'mail' ) {
		...
		$subject = new KiwiCfMailTaggedText( $subject,
			array( 'callback' => $callback ) );
		$subject = $subject->replace_tags();
		$subject = kiwi_cf_strip_newline( $subject );
		$this->detect_maybe_empty( sprintf( '%s.subject', $template ), $subject );

		$sender = $components['sender'];
		$sender = new KiwiCfMailTaggedText( $sender,
			array( 'callback' => $callback ) );
		$sender = $sender->replace_tags();
		$sender = kiwi_cf_strip_newline( $sender );

		if ( ! $this->detect_invalid_mailbox_syntax( sprintf( '%s.sender', $template ), $sender )
		and ! kiwi_cf_is_email_in_site_domain( $sender ) ) {
			...
			if ( ! preg_match( '/^([0-9A-Za-z-]+):(.*)$/', $header, $matches ) ) {
				...
				if ( in_array( strtolower( $header_name ), $mailbox_header_types ) ) {
					$this->detect_invalid_mailbox_syntax(
						...
						$header_value, array(
							'message' =>
								__( "Invalid mailbox syntax is used in the %name% field.", 'kiwi-contact-form' ),
		...
		$body = new KiwiCfMailTaggedText( $body,
			array( 'callback' => $callback ) );
		...
		if ( '' !== $components['attachments'] ) {
			...
			if ( $max < $total_size ) {
				$this->add_error( sprintf( '%s.attachments', $template ),
					...
					array(
						'message' => __( "The total size of attachment files is too large.", 'kiwi-contact-form' ),
	...
	public function detect_invalid_mailbox_syntax( $section, $content, $args = '' ) {
		...
		if ( ! kiwi_cf_is_mailbox_list( $content ) ) {
			return $this->add_error( $section,
	...
	public function detect_file_not_found( $section, $content ) {
		...
		or ! is_file( $path ) ) {
			return $this->add_error( $section,
				...
				array(
					'message' =>
						__( "Attachment file does not exist at %path%.", 'kiwi-contact-form' ),
	...
	public function detect_file_not_in_content_dir( $section, $content ) {
		...
		if ( ! kiwi_cf_is_file_path_in_content_dir( $path ) ) {
			return $this->add_error( $section,
				...
				array(
					'message' =>
						__( "It is not allowed to use files outside the wp-content directory.", 'kiwi-contact-form' ),
