...
function kiwi_cf_is_valid_locale( $locale ) {
	$pattern = '/^[a-z]{2,3}(?:_[a-zA-Z_]{2,})?$/';
	return (bool) preg_match( $pattern, $locale );
}

function kiwi_cf_is_rtl( $locale = '' ) {
...
function kiwi_cf_load_textdomain( $locale = null ) {
	global $l10n;

	$domain = 'kiwi-contact-form';
