...
add_action( 'kiwi_cf_submit', 'kiwi_cf_flamingo_submit', 10, 2 );

function kiwi_cf_flamingo_submit( $contact_form, $result ) {
	...
	$cases = (array) apply_filters( 'kiwi_cf_flamingo_submit_if',
	...
	$email = kiwi_cf_flamingo_get_value( 'email', $contact_form );
	$name = kiwi_cf_flamingo_get_value( 'name', $contact_form );
	$subject = kiwi_cf_flamingo_get_value( 'subject', $contact_form );
	...
	foreach ( $special_mail_tags as $smt ) {
		$meta[$smt] = apply_filters( 'kiwi_cf_special_mail_tags', '',
	...
	do_action( 'kiwi_cf_after_flamingo', $result );
}

function kiwi_cf_flamingo_get_value( $field, $contact_form ) {
	if ( empty( $field )
	...
	if ( in_array( $field, array( 'email', 'name', 'subject' ) ) ) {
		...
		if ( empty( $templates[0] ) ) {
			...
			$template = trim( kiwi_cf_strip_quote( $templates[0] ) );
		}

		$value = kiwi_cf_mail_replace_tags( $template );
	}

	$value = apply_filters( 'kiwi_cf_flamingo_get_value', $value,
		$field, $contact_form );

	return $value;
}

function kiwi_cf_flamingo_add_channel( $slug, $name = '' ) {
	if ( ! class_exists( 'Flamingo_Inbound_Message' ) ) {
		return false;
	}

	$parent = term_exists( 'kiwi-contact-form',
		Flamingo_Inbound_Message::channel_taxonomy );

	if ( ! $parent ) {
		$parent = wp_insert_term( __( 'Kiwi Contact Form', 'kiwi-contact-form' ),
			Flamingo_Inbound_Message::channel_taxonomy,
			array( 'slug' => 'kiwi-contact-form' ) );
...
add_action( 'kiwi_cf_after_update', 'kiwi_cf_flamingo_update_channel', 10, 1 );

function kiwi_cf_flamingo_update_channel( $contact_form ) {
	if ( ! class_exists( 'Flamingo_Inbound_Message' ) ) {
		return false;
	}
...
add_filter( 'kiwi_cf_special_mail_tags', 'kiwi_cf_flamingo_serial_number', 10, 3 );

function kiwi_cf_flamingo_serial_number( $output, $name, $html ) {
	...
	if ( ! $contact_form = Kiwi_CF_ContactForm::get_current() ) {
		return $output;
	}
