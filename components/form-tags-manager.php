...
function kiwi_cf_add_form_tag( $tag, $func, $features = '' ) {
	$manager = KiwiCfFormTagsManager::get_instance();

	return $manager->add( $tag, $func, $features );
}

function kiwi_cf_remove_form_tag( $tag ) {
	$manager = KiwiCfFormTagsManager::get_instance();

	return $manager->remove( $tag );
}

function kiwi_cf_replace_all_form_tags( $content ) {
	$manager = KiwiCfFormTagsManager::get_instance();

	return $manager->replace_all( $content );
}

function kiwi_cf_scan_form_tags( $cond = null ) {
	$contact_form = KiwiCfContactForm::get_current();
...
function kiwi_cf_form_tag_supports( $tag, $feature ) {
	$manager = KiwiCfFormTagsManager::get_instance();

	return $manager->tag_type_supports( $tag, $feature );
}

class KiwiCfFormTagsManager {
	...
	private function scan_callback( $m, $replace = false ) {
		...
		if ( is_array( $attr ) ) {
			if ( is_array( $attr['options'] ) ) {
				...
				and ! empty( $attr['options'] ) ) {
					...
					if ( ! kiwi_cf_is_name( $scanned_tag['name'] ) ) {
			...
			if ( KIWI_CF_USE_PIPE ) {
				$pipes = new Kiwi_CF_Pipes( $scanned_tag['raw_values'] );
		...
		$scanned_tag = apply_filters( 'kiwi_cf_form_tag', $scanned_tag, $replace );

		$scanned_tag = new KiwiCfFormTag( $scanned_tag );
	...
	private function parse_atts( $text ) {
		...
		if ( preg_match( $pattern, $text, $match ) ) {
			...
			if ( ! empty( $match[2] ) ) {
				...
				$atts['values'] = kiwi_cf_strip_quote_deep( $matched_values[0] );
			}
		} else {
			$atts = $text;
		}
