...
class Kiwi_CF_Validation implements ArrayAccess {
	...
	public function invalidate( $context, $message ) {
		if ( $context instanceof KiwiCfFormTag ) {
			$tag = $context;
		} elseif ( is_array( $context ) ) {
			$tag = new KiwiCfFormTag( $context );
		} elseif ( is_string( $context ) ) {
			$tags = kiwi_cf_scan_form_tags( array( 'name' => trim( $context ) ) );
			$tag = $tags ? new KiwiCfFormTag( $tags[0] ) : null;
		}

		$name = ! empty( $tag ) ? $tag->name : null;

		if ( empty( $name )
		or ! kiwi_cf_is_name( $name ) ) {
			return;
		}
